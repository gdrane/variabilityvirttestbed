...
// Frequency change handler added by gdrane
static void frequency_change_handler(void* opaque, int irq, int level)
{
	ssys_state* s = (ssys_state*) opaque;
	struct cycle_counter cur_count, chkpt;
	struct energy_counter e_count;
	read_all_cycle_counters(&cur_count);
	read_all_cycle_counter_chkpts(&chkpt);
	e_count.data_proc_energy = s->rcc * (cur_count.data_proc_cycles - chkpt.data_proc_cycles);
	e_count.branch_energy = s->rcc * (cur_count.branch_cycles - chkpt.branch_cycles);
	e_count.multiply_energy = s->rcc * (cur_count.multiply_cycles - chkpt.multiply_cycles);
	e_count.ldst_energy = s->rcc * (cur_count.ldst_cycles - chkpt.ldst_cycles);
	e_count.misc_energy = s->rcc * (cur_count.misc_cycles - chkpt.misc_cycles);
	freq_changed_set_chkpt(&cur_count);	
}
...
static void ssys_write(void *opaque, target_phys_addr_t offset, uint32_t value)
{
    ...
    case 0x060: /* RCC */
		...
		printf("Value RCC%d\n", s->rcc);
        if(value != s->rcc)
			frequency_change_handler(s, 1, 1);
		s->rcc = value;
...
static ssys_state* stellaris_sys_init(uint32_t base, qemu_irq irq,
                              stellaris_board_info * board,
                              uint8_t *macaddr)
{
    int iomemtype;
	...
	// s->frequency_change_irq = qemu_allocate_irqs(frequency_change_handler, (void*)s, 1);
	return s;
}
...
{	
	...
	read_curr_active_energy(&active);
	read_curr_sleep_energy(&sleep);
                           ...
                           stellaris_board_info *board)
{
    ...
    sys_cntrl = stellaris_sys_init(0x400fe000, pic[28], board, nd_table[0].macaddr.a);
